...
/**
 * @author Jonah Ryan (dijkstrasShortestPath method)
 * 
 * Weighted directed graph implementation provided by course staff
 */
public class CS400Graph<T> implements GraphADT<T> {
   ...
   */
	protected class Vertex {
	    public T data; // Vertex label (the name of the location) used for interacting with each node
	    public NodeData info; // vertex info with more data about the vertex
	    public LinkedList<Edge> edgesLeaving;

	    public Vertex(T data, NodeData info) {
	      this.data = data;
	      this.info = info;
   ...
   * 
   * @param data the name of the data item to store into the graph
   * @param info the NodeData object corresponding with the stored object name.
   * @return true if the data can be inserted as a new vertex, false if it is already in the graph
   * @throws NullPointerException if data is null
   */
  public boolean insertVertex(T data, NodeData info) {
    if (data == null)
    ...
    vertices.put(data, new Vertex(data, info));
  ...
  public boolean removeVertex(T data) {
    ...
    for (Vertex v : vertices.values()) {
      ...
      for (Edge e : v.edgesLeaving){
        if (e.target == removeVertex){ removeEdge = e; }
      }

      // Remove any such edges that are found
      if (removeEdge != null){
  ...
  /**
   * Helper method to return a vertex that contains any inputted data
   * 
   * @param data the data item to check for
   * @return the matching vertex
   * @throws NoSuchElementException if data does not match any vertices
   */
  public Vertex getVertex(T data) {
    if (data == null)
      throw new NoSuchElementException("Data point not contained in the graph!");
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      this.start = copyPath.start; // Copy Starting path
      this.distance = copyPath.distance + extendBy.weight; // Find new cost
      this.dataSequence = new LinkedList<>();
      this.end = extendBy.target; // Set Ending to the newest edges vertex

      /*
       * Copy the old DataSequence without overwriting the old Sequence.
       * 
       * Using a for loop instead of the .clone method to keep the datSequence a LinkedList. As you
       * would have to cast to a List data type with .clone.
       */
      ...
      // Adds the newest data to the dataSequence.
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    Hashtable<T, Boolean> visited = new Hashtable<T, Boolean>(); // Stores if a node was visited
    PriorityQueue<Path> paths = new PriorityQueue<Path>(); // Stores the paths to loop through
    Hashtable<T, Path> keyPaths = new Hashtable<T, Path>(); // Stores the quickest path to a node

    // Add the start node to the keyPaths and the Priority Queue
    // A NoSuchElementException will be thrown if "start" does not match any vertex.
    keyPaths.put(start, new Path(getVertex(start)));
    paths.add(new Path(getVertex(start)));


    // Start the main Dijkstra algorithm
    while (!paths.isEmpty()) {
      Path current = paths.remove();
      visited.put(current.end.data, true); // Update the visited list

      // Go through every successor of the current node, this being the end node of the current path
      for (Edge e : current.end.edgesLeaving) {

        // Check if the successor has not been visited, if not null, this node will be skipped.
        if (visited.get(e.target.data) == null) {

          // Get the new distance to this successor in respect to the current node
          double newDis = keyPaths.get(current.end.data).distance + e.weight;

          // Check if the successor's saved keyPath is reducible, if they have no keyPath, continue
          if (keyPaths.get(e.target.data) == null
              || keyPaths.get(e.target.data).distance > newDis) {

            // Update to this quicker path and add the node to the Priority Queue for a later loop
            keyPaths.put(e.target.data, new Path(keyPaths.get(current.end.data), e));
            paths.add(new Path(keyPaths.get(current.end.data), e));
          }
        }
      }
    }
    ...
    /*
     * Now that the loop has finished, return the end nodes keyPath. If no keyPath was set, throw a
     * NoSuchElementException exception as their was no path to the node found.
     */
    Path toReturn = keyPaths.get(end);
    if (toReturn == null) {
      throw new NoSuchElementException("No path was found between the start and end node.");
    }
    return toReturn;
  }
